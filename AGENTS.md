...
3. **Leaderboard screen** (`_leaderboard_screen.html`) — top-25 table with date header and madlib cycling; shown on `GET /leaderboard-screen`. Winning entries are saved automatically by `POST /guess` (no manual button).
...
| `POST /guess` | Send guess via workflow Update; on a win, auto-save the leaderboard entry to SQLite (email + game_date); return board partial fragment |
...
- **`workflow.py`**: `UserSessionWorkflow` — `run()` selects a word for Daily/Random or initializes sorted `ANSWER_LIST` candidates for Absurdle, `make_guess` Update handler with `wait_condition` guard for init race, `validate_make_guess` validator uses `WORD_LENGTH`, `get_game_state` Query handler
...
- Querying a completed workflow still works (status `COMPLETED` in `_query_existing_game`) — used by `GET /board` and `/share` to read a finished game
